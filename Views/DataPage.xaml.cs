...
namespace TapPay.Views
{
    public partial class DataPage : ContentPage
    {
        ...
        private async void LoadData()
        {
            ...
            List<Organizador> organizadores = await App.Database.GetOrganizadoresAsync();
            List<Evento> eventos = await App.Database.GetEventosAsync();
            List<Usuario> Usuarios = await App.Database.GetUsuariosAsync();
            List<Producto> productos = await App.Database.GetProductosAsync();
            List<Cliente> clientes = await _databaseService.GetClientesAsync(usuario_id);
